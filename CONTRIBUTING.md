...
3. 运行 `tools/render_lucky_artifacts.py` 从证据生成 Markdown 和 OpenAPI。
4. 审核生成 diff，特别注意 URL、token、域名、配置值和风险等级变化。
5. 运行 `python3 -m unittest discover -s tests -v` 和 `python3 tools/verify_repository.py`。
6. 在 PR 中写明 Lucky 版本、镜像类型、bundle 数量和只读实测范围。
