...
- 内置客户端综合前端静态快照和与其 SHA-256 精确绑定的证据覆盖做模板匹配、风险分级和 schema 补全；当前 Lucky 3.0.0 默认目录为 0 条 unknown，非 Docker 请求体 schema 缺口为 0，仍保留 6 条无法可靠恢复的 Docker legacy wrapper 缺口。未收录端点、写请求及危险 GET 仍默认拒绝。
